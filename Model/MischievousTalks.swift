...
class MischievousTalks {
    
    enum Talk: Int {
        case dirty, alluring
    }
    
    static func whistleFor(talk: Talk) -> String {
        let whistles = stubs[talk.rawValue]
        return whistles[Int.random(in: 0 ..< whistles.count)]
    }
